...
def top5validate_unet(val_loader, device, model, criterion):
    # validation
    model.eval()
    L1val_accum = 0.0
    ...
    return float(L1val_accum)

def top5validate_diff_unet(val_loader, device, model, criterion):
    # validation
    model.eval()
    L1val_accum = 0.0
    modelConfig = {
    ...
    for i, validata in enumerate(val_loader, 0):
        ...
        condition = inputs.to(device)
        loss = sampler(x_0, condition).sum() / 1000.
        lossL1viz = loss.item()
        L1val_accum += lossL1viz

    return float(L1val_accum)

def reward_caculation_unet(pruned_model, val_loader, device, sum_out_c):
    # here we do the pseudo pruning, by zero mask those pruned channel
    # 伪剪枝 通过掩码0来实现剪枝
    # nn.L1LOss() 对数组每个元素作差求和除以数组的元素个数
    criterion = nn.L1Loss().to(device)
    L1val_accum = top5validate_unet(val_loader, device, pruned_model, criterion)
    # print("========reward==========")
    # print(L1val_accum) #2.44769
    # print(len(val_loader)) #40
    # print(sum_out_c) #1000

    # reward 1
    # reward = ((L1val_accum / len(val_loader)) * 100 + sum_out_c * 0.001) * -1

    # reward 2
    # reward = ((L1val_accum / len(val_loader)) * 300 + sum_out_c * 0.001) * -1

    # reward 3
    # reward = ((L1val_accum / len(val_loader)) * 200 + sum_out_c * 0.001) * -1

    # reward 4  unet-studnet
    reward = ((L1val_accum / len(val_loader)) * 150 + sum_out_c * 0.0015) * -1


    return float(reward)

def reward_caculation_diff_unet(pruned_model, val_loader, device):
    # here we do the pseudo pruning, by zero mask those pruned channel
    # 伪剪枝 通过掩码0来实现剪枝
    criterion = nn.L1Loss().to(device)
    L1val_accum = top5validate_diff_unet(val_loader, device, pruned_model, criterion)
    reward = L1val_accum * -100
    return float(reward)
